 ...
 */
import {
  DATA_CLASS, ENVIRONMENT_ID, RISK_TIER, RISK_TIER_ORDER,
  type DataClass, type EnvironmentId, type RiskTier,
} from '../types/index.js';
import type { CapabilityRegistry } from './capability-registry.js';

function elevateRiskTier(current: RiskTier, minimum: RiskTier): RiskTier {
  return RISK_TIER_ORDER.indexOf(current) >= RISK_TIER_ORDER.indexOf(minimum)
    ? current
    : minimum;
}
