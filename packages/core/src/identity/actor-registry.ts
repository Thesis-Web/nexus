...
import {
  ACTOR_CLASS,
  type Actor, type Uuid, type ActorClass,
  type ActorRegistryStore,
} from '../types/index.js';

interface ActorRow {
  actor_id: string; actor_class: string; principal_id: string;
  display_name: string; environment: string; risk_ceiling: string;
  allowed_systems: string; registered_at: string;
  owner: string | null; purpose: string | null; review_cadence: string | null;
}
...
export class ActorRegistry implements ActorRegistryStore {
  ...
  async register(actor: Actor): Promise<Actor> {
    ...
    this.db
      ...
      `)
      .run(
        actor.actorId, actor.actorClass, actor.principalId,
        actor.displayName, actor.environment, actor.riskCeiling,
        JSON.stringify(actor.allowedSystems), actor.registeredAt,
        actor.owner ?? null, actor.purpose ?? null, actor.reviewCadence ?? null
      );
    return actor;
  }
