...
export async function mintSubDelegation(
  ...
  if (!parentInStore) {
    throw new DelegationChainIntegrityError(
      `chain_integrity_broken: parent delegation '${parentDelegation.delegationId}' not found in store`
    );
  }
  ...
  if (newChainDepth > parentDelegation.maxChainDepth) {
    throw new DelegationError(
      `chain_depth_ceiling_exceeded: depth ${newChainDepth} exceeds max ${parentDelegation.maxChainDepth}`
    );
  }
