...
export interface PendingApprovalStore {
  create(approval: {
    approvalId: Uuid; actionId: Uuid; templateId: Uuid;
    requestJson: string; channelId: string;
    dispatchedAt: IsoTimestamp; expiresAt: IsoTimestamp;
  }): Promise<void>;
  ...
  listPending(): Promise<Array<{ approvalId: string; channelId: string; expiresAt: string; requestJson: string }>>;
}
...
export class SqlitePendingApprovalStore implements PendingApprovalStore {
  ...
  async create(approval: {
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')
    `).run(
      approval.approvalId, approval.actionId, approval.templateId,
      approval.requestJson, approval.channelId, approval.dispatchedAt, approval.expiresAt
    );
  }

  async getStatus(approvalId: Uuid): Promise<{ status: string; responseJson: string | null } | null> {
    const row = this.db
  ...
  async resolve(approvalId: Uuid, status: 'approved' | 'denied', responseJson: string): Promise<void> {
    this.db.prepare(
      'UPDATE pending_approvals SET status = ?, response_json = ? WHERE approval_id = ?'
    ).run(status, responseJson, approvalId);
  }

  async markTimedOut(approvalId: Uuid): Promise<void> {
    this.db.prepare(
      "UPDATE pending_approvals SET status = 'timed_out' WHERE approval_id = ?"
    ).run(approvalId);
  }

  async listPending(): Promise<Array<{ approvalId: string; channelId: string; expiresAt: string; requestJson: string }>> {
    const rows = this.db
      .prepare("SELECT approval_id, channel_id, expires_at, request_json FROM pending_approvals WHERE status = 'pending'")
      .all() as Array<{ approval_id: string; channel_id: string; expires_at: string; request_json: string }>;
    return rows.map(r => ({
      approvalId: r.approval_id, channelId: r.channel_id,
      expiresAt: r.expires_at, requestJson: r.request_json,
    }));
  }
}
