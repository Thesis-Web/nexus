 ...
 */
import { APPROVAL_DECISION_LABEL, } from '../types/index.js';
...
export async function decideApproval(approvalId, approverId, decision, note, store) {
    // 1. Load pending record
    const row = await store.getStatus(approvalId);
    if (!row)
        throw new ApprovalDecisionError(`Approval ${approvalId} not found`);
    ...
    if (!requestJson)
        throw new ApprovalDecisionError(`Approval ${approvalId} request not found`);
    ...
    const response = { ...responseBody, signature };
    // 8. Persist
