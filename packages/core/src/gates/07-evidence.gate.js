 ...
 */
import { GATE_ID, GENESIS_HASH, OUTCOME_LABEL, FINAL_OUTCOME, DENIAL_CODE, } from '../types/index.js';
...
function buildMinimalDelegationSnapshot(dc) {
    return {
...
function buildGrantMetadata(grant, template) {
    return {
...
function computeFinalOutcome(decisions) {
    for (const d of decisions) {
        if (d.outcome === 'pass' ||
            ...
            continue;
        if (d.outcome === 'error')
            return FINAL_OUTCOME.ERROR;
        const code = d.denialCode;
        if (code === DENIAL_CODE.APPROVAL_TIMEOUT)
            return FINAL_OUTCOME.DENIED_TIMEOUT;
        ...
        if (d.gateId === GATE_ID.G06 && d.outcome === 'deny')
            return FINAL_OUTCOME.DENIED_THREAT;
        if (d.gateId === GATE_ID.G01)
            return FINAL_OUTCOME.DENIED_IDENTITY;
        if (d.gateId === GATE_ID.G02)
            return FINAL_OUTCOME.DENIED_CLASSIF;
        if (d.gateId === GATE_ID.G03)
            return FINAL_OUTCOME.DENIED_DELEGATION;
        if (d.gateId === GATE_ID.G04)
            return FINAL_OUTCOME.DENIED_POLICY;
        if (d.gateId === GATE_ID.G05)
            return FINAL_OUTCOME.DENIED_APPROVAL;
    }
...
export class EvidenceGate {
    ...
    gateOrder = 7;
    plane = 'control';
    constructor(ledger, controlPlaneKey) {
        this.ledger = ledger;
        this.controlPlaneKey = controlPlaneKey;
    }
    async evaluate(action, context, decisions) {
        ...
        const recordBodyPreCCV = {
            ...
            policyOutcome: policyDecision ? policyDecision.outcome : null,
        ...
        const record = { ...recordBodyFull, recordHash, signature };
