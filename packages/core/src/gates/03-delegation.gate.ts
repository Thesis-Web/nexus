 ...
 */
import {
  GATE_ID, DENIAL_CODE, ACTOR_CLASS,
  DelegationChainIntegrityError,
  riskTierExceeds,
  type Gate, type GateResult, type AgentAction,
  type PipelineContext, type GateDecision, type DelegationContextSnapshot,
} from '../types/index.js';
...
function deny(code: string, reason: string, startMs: number): GateResult {
  return {
    decision: {
      gateId: GATE_ID.G03, gateOrder: 3, plane: 'control',
...
export class DelegationGate implements Gate {
  ...
  async evaluate(
    ...
    if (!isValid) return deny(DENIAL_CODE.DELEGATION_SIG_INVALID, 'delegation signature invalid', startMs);
    ...
    return {
      decision: {
        gateId: GATE_ID.G03, gateOrder: 3, plane: 'control',
        outcome: 'pass', reason: 'delegation valid',
...
async function buildDelegationSnapshot(
  ...
  while (current.parentDelegationId !== null) {
    ...
    if (!parent) throw new DelegationChainIntegrityError(
