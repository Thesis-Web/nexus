 ...
 */
import { GATE_ID, DENIAL_CODE, APPROVAL_DECISION_LABEL, } from '../types/index.js';
import { buildSignedApprovalRequest } from '../approval/packager.js';
import { verify } from '../crypto/verifier.js';
function deny(code, reason, startMs, req, resp) {
    return {
        decision: {
            gateId: GATE_ID.G05,
            gateOrder: 5,
...
export class ApprovalGate {
    ...
    async evaluate(action, context, _prior) {
        ...
        const signedRequest = await buildSignedApprovalRequest(action, template, context, diff, this.controlPlaneKey);
        ...
        if (response.decidedBy !== 'system:timeout') {
            ...
            if (approver) {
                ...
                const valid = await verify(JSON.stringify(body).replace(/"signature":"[^"]*"/, ''), signature, approver.publicKey);
