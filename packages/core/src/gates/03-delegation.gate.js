 ...
 */
import { GATE_ID, DENIAL_CODE, ACTOR_CLASS, DelegationChainIntegrityError, riskTierExceeds, } from '../types/index.js';
...
export class DelegationGate {
    ...
    async evaluate(action, context, _prior) {
        ...
        if (!dc.allowedCapabilities.includes(action.resolvedCapability)) {
            return deny(DENIAL_CODE.CAPABILITY_NOT_IN_DELEGATION, 'capability not in delegation', startMs);
        }
        ...
        if (dc.parentDelegationId !== null && !dc.allowDownstreamPropagation) {
            return deny(DENIAL_CODE.PROPAGATION_NOT_PERMITTED, 'downstream propagation not permitted', startMs);
        }
        // Environment must match — blueprint drift prevention law
        if (action.resolvedTarget.environment !== dc.environment) {
            return deny(DENIAL_CODE.ENVIRONMENT_MISMATCH, `environment mismatch: delegation scoped to ${dc.environment}, action targets ${action.resolvedTarget.environment}`, startMs);
        }
        // Build chain snapshot — throws DelegationChainIntegrityError if parent missing
        let delegationSnapshot;
        try {
...
async function buildDelegationSnapshot(dc, context) {
    const ancestors = [];
    let current = dc;
    while (current.parentDelegationId !== null) {
        ...
        if (!parent)
            throw new DelegationChainIntegrityError(`Delegation chain broken: parent ${current.parentDelegationId} not found in store`);
        current = parent;
    }
    return {
