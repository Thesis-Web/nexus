...
function makePolicy(outcome: string, conditions: Record<string, unknown> = {}): LoadedPolicyFile {
  ...
  return {
    ...
    createdAt: NOW, signature: 'sig', policyHash: 'hash', bundleHash: 'bundlehash',
...
describe('Gate 04 — Policy', () => {
  ...
  it('denies POLICY_DENY when matched rule outcome is deny', async () => {
    const result = await new PolicyGate().evaluate(baseAction(), makeCtx(makePolicy(OUTCOME_LABEL.DENY)), []);
    expect(result.decision.outcome).toBe('deny');
  ...
  it('returns allow outcome and sets grantTemplate when matched rule is allow', async () => {
    const result = await new PolicyGate().evaluate(baseAction(), makeCtx(makePolicy(OUTCOME_LABEL.ALLOW)), []);
  ...
  it('returns require_approval outcome — orchestrator routes to Gate 05', async () => {
    const result = await new PolicyGate().evaluate(baseAction(), makeCtx(makePolicy(OUTCOME_LABEL.REQUIRE_APPROVAL)), []);
