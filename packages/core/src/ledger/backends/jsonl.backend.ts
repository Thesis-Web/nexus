...
export class JsonlLedgerBackend implements LedgerBackend {
  ...
  async listRange(from: number, to: number): Promise<EvidenceRecord[]> {
    ...
    for (const line of raw.split('\n').filter(Boolean)) {
      try {
        ...
        // LEDGER-001 FIX: fail closed — malformed evidence record is a data integrity violation
        throw new Error(
          `[JsonlLedgerBackend] malformed evidence ledger line in listRange: ${(err as Error).message}`
        );
      }
    }
    return results;
  ...
  async getByRecordId(recordId: Uuid): Promise<EvidenceRecord | null> {
    ...
    for (const line of raw.split('\n').filter(Boolean)) {
      try {
        ...
        // LEDGER-001 FIX: fail closed — malformed evidence record is a data integrity violation
        throw new Error(
          `[JsonlLedgerBackend] malformed evidence ledger line in getByRecordId: ${(err as Error).message}`
        );
      }
    }
    return null;
  ...
  async getLatestSequence(): Promise<number> {
    ...
    try {
      ...
      // LEDGER-001 FIX: fail closed — malformed evidence record is a data integrity violation
      throw new Error(
        `[JsonlLedgerBackend] malformed evidence ledger line in getLatestSequence: ${(err as Error).message}`
      );
    }
  }
}
