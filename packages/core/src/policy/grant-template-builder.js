 ...
 */
import { OUTCOME_LABEL, EXPIRY_CLASS, EXPIRY_CLASS_SECONDS, NexusSecurityViolation, DENIAL_CODE, } from '../types/index.js';
 ...
 */
export function templateFingerprintPayload(template) {
    return stripUndefined({ ...template });
}
export function assertTemplateIntegrity(template) {
...
function resolveCredentialSubjectType(actorClass, _system) {
    if (actorClass === 'HUMAN' || actorClass === 'HUMAN_WITH_COPILOT')
        return 'user_identity';
    return 'service_identity';
}
function buildScopeDescriptor(capabilityId, target, hint) {
...
export function buildGrantTemplate(action, rule, context) {
    ...
    const approvalRequired = rule?.outcome === OUTCOME_LABEL.REQUIRE_APPROVAL || rule?.outcome === OUTCOME_LABEL.ESCALATE;
    const templateBody = {
        templateId: newUuid(),
