...
export function canonicalize(val) {
    if (val === null)
        return 'null';
    if (val === undefined) {
        throw new TypeError('canonicalize: undefined is not a legal canonical value — use null or omit the field');
    }
    ...
    if (Array.isArray(val)) {
        return ('[' +
            val
                .map(v => {
                if (v === undefined) {
                    throw new TypeError('canonicalize: undefined element in array — use null or remove the element');
                }
