 ...
 */
export async function loadControlPlaneKey() {
 ...
 */
export async function loadApproverKey(approverId) {
    ...
    try {
        ...
        throw new Error(`Approver key not found for approverId '${approverId}': ${err.message}`);
    }
}
/**
 ...
 */
export async function generateApproverKeypair(approverId) {
 ...
 */
export async function generateControlPlaneKeypair() {
    const privBytes = ed25519.utils.randomPrivateKey();
    const pubBytes = await ed25519.getPublicKeyAsync(privBytes);
    const pair = {
        publicKey: base64urlEncode(pubBytes),
