...
# Version: v0.2.1 (DRAFT — pending owner ratification)
# Date: 2026-05-12
# Status: DRAFT — incorporates owner-approved best-solve rows from v0.1.0 review (audit 1 + audit 2 + owner pushback on rows 8, 10) + v0.2.0→v0.2.1 export-path-discipline addition
# Amends:
...
## Changelog from v0.2.0 (v0.2.1 delta)

- **Export-path discipline (NEW §3.1.1).** Owner pushback on v0.2.0: the prose distinction "hot path uses inverted map; offline audit uses deterministic decode" is enforced by where the decode function is exported FROM, not by the comment next to it. Without structural separation, intent decays. Added §3.1.1 specifying the audit-only export path, allowed-importer list, and a ci:gate import-boundary check. Cross-referenced from §5.1. Added §9.1 boundary test. Updated §11 risk wording from "mitigation: deterministic derivation + comment" to "mitigation: structural export-path separation enforced by ci:gate."
...
### §3.1.1 Export-path discipline (the hot-path / audit-only boundary IS the export structure)

The provenance authority law in §3.1 says compile resolves provenance via the typed `MailboxAllocation` record + `MailboxService.resolveMailboxProvenance` on the hot path. The deterministic derivation function (`decodeActorIdFromMailboxId(mailboxId): Uuid | null`) exists ONLY for offline audit tooling — given a mailbox file on disk, recover the actor without the live service.

The "offline only" intent MUST be enforced by the **export path**, not by a comment in the source file. A comment that says "do not use in hot path" decays within two build cycles to "everyone imports it because it's convenient." Structural separation prevents the drift.

#### §3.1.1.1 Module placement

| Symbol | Module | Public barrel? |
|---|---|---|
| `MailboxAllocation` (type) | `packages/contracts/src/externals/mailbox.ts` | YES — `@nexus/contracts` re-exports it |
| `MailboxService.resolveMailboxProvenance` (method) | `packages/contracts/src/externals/mailbox.ts` | YES — part of `MailboxService` interface |
| `MailboxService.assertMailboxBelongsToActor` (method) | `packages/contracts/src/externals/mailbox.ts` | YES — same |
| `MailboxServiceImpl.allocateForRun` / `getMailboxForActor` / `listMailboxesForRun` (methods) | `packages/core/src/mailbox/mailbox-service.ts` | YES via `MailboxService` interface |
| **`decodeActorIdFromMailboxId(mailboxId): Uuid \| null`** (deterministic deriver) | **`packages/core/src/mailbox/mailbox-audit-utils.ts`** | **NO** — not re-exported from `packages/core/src/mailbox/index.ts`; not re-exported from `packages/core/src/index.ts`; not re-exported from anywhere consumed by `scripts/`, `packages/orch-ref/`, `packages/vanguard/`, `packages/interfaces/`, or `packages/workspace-ref/` |

#### §3.1.1.2 Allowed importers

The function `decodeActorIdFromMailboxId` may be imported ONLY by:

1. Audit CLI scripts under `tools/audit/**` (none today; reserved path).
2. Unit tests for the audit utilities themselves at `packages/core/src/mailbox/mailbox-audit-utils.test.ts`.
3. Future approved audit-only modules explicitly added to this list by amendment.

No other module — production or test — may import this symbol. The audit utilities module path itself is the gate: importing from `mailbox-audit-utils.ts` is the deliberate act that signals "I am offline tooling."

#### §3.1.1.3 ci:gate enforcement

A new ci:gate check (`MAILBOX-PIT-EXPORT-BOUNDARY` — name TBD at build time) scans for imports of `decodeActorIdFromMailboxId` across the repo. The check fails closed on any import outside the allowed list in §3.1.1.2.

Rationale (saved as memory `feedback_export_path_is_enforcement.md`): the export path is the enforcement mechanism, not the comment. Documentation alone has consistently failed to hold this kind of boundary across build cycles. The ci:gate scan converts the intent into a mechanical check.

#### §3.1.1.4 Why this is in the spec, not just the build

This rule is structural law, not implementation polish. Future amendments that touch mailbox naming (or add new audit-only utilities elsewhere in the stack) inherit the discipline by referencing §3.1.1. The pattern generalizes: any restricted-use function in any module should follow the same export-path discipline.
...
`MailboxService.resolveMailboxProvenance` exists for production code paths that need a typed `MailboxAllocation` record (e.g., a future audit-aware compile mode). Hot path uses the inverted map.

The **deterministic decode function** `decodeActorIdFromMailboxId` (offline-only) lives in `packages/core/src/mailbox/mailbox-audit-utils.ts` and is NOT exported through any public barrel. Compile MUST NOT import it. This boundary is enforced structurally per §3.1.1 (export-path discipline) and verified by the `MAILBOX-PIT-EXPORT-BOUNDARY` ci:gate scan.
...
- `mailbox-audit-utils.decodeActorIdFromMailboxId` — happy path, malformed input returns null, prefix mismatch returns null. Tests live at `packages/core/src/mailbox/mailbox-audit-utils.test.ts` (the test file is itself one of the allowed importers per §3.1.1.2).

### §9.1.1 Export-boundary ci:gate test

New ci:gate check `MAILBOX-PIT-EXPORT-BOUNDARY` scans every TypeScript file in the repo for imports of `decodeActorIdFromMailboxId`. Allowed importers (§3.1.1.2): `tools/audit/**`, `packages/core/src/mailbox/mailbox-audit-utils.test.ts`. Any other importer fails the gate. The gate also verifies the symbol is NOT re-exported from `packages/core/src/mailbox/index.ts`, `packages/core/src/index.ts`, or `packages/contracts/src/**`.
...
- **Allocation persistence on restart.** If persistence layer fails OR ledger events are missing, `listMailboxesForRun` returns incomplete data. Mitigation: deterministic derivation + ledger reconstruction as fallback (the deterministic derivation logic itself lives in the audit-utils module per §3.1.1 — the service-internal use is allowed but the function symbol must not leak out of the audit-utils export boundary). Test: §9.3 restart test.

- **Provenance derivation drift.** v0.2.0 mitigation was "use the typed service on hot path, deterministic decode for offline." Without structural separation, the deterministic decode gets imported into hot paths within two build cycles as a convenience. **Mitigation (v0.2.1):** `decodeActorIdFromMailboxId` lives in `mailbox-audit-utils.ts` and is NOT exported through any public barrel. ci:gate `MAILBOX-PIT-EXPORT-BOUNDARY` enforces the import-boundary mechanically (§3.1.1.3). The export path IS the enforcement; the comment is not.
...
ADD-MAILBOX-PIT-002 | spec: §3.1.1 + §9.1.1 | infra-externals (no prior section) | New ci:gate `MAILBOX-PIT-EXPORT-BOUNDARY` enforces the audit-utils import boundary — decodeActorIdFromMailboxId must not appear in any importer outside the §3.1.1.2 allowlist, and must not be re-exported from any public barrel | required to make the offline-only intent structural rather than advisory; comment-only enforcement decays within build cycles | downstream: build-phase ci-gate.ts addition; reviewer mechanical check | status: pending build phase
