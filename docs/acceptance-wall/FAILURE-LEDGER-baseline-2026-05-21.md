...
Generated: 2026-05-21T15:00:40.583Z
...
| class | count |
| --- | --- |
| UNIMPLEMENTED_TEST_BODY | 43 |
| EXTERNAL_DEPENDENCY | 31 |
| PRODUCT_RUNTIME | 24 |
| UNIMPLEMENTED_SURFACE | 20 |
| UNCLASSIFIED | 1 |
...
### `NXS-DISPATCH-BRIDGE-RETURNS-NULL` — blocks 24 tests
...
### `E2E-MULTI-NO-CONTRACT-CATALOG` — blocks 11 tests
...
### `E2E-CHAT-ONPREM-CATALOG` — blocks 9 tests
...
### `E2E-RBAC-DIFFERENTIALS-CATALOG` — blocks 8 tests
...
### `E2E-HARD-LAW-CATALOG` — blocks 8 tests
...
### `F4.12-COMPILE-MULTI-ITEM-PASSTHROUGH` — blocks 1 test
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-02 — `E2E-02-chat-time: intern → "what time is it where you are?"`
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-03 — `E2E-03-chat-math: intern → "what is 17 times 23?"`
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-04 — `E2E-04-chat-summarize: analyst → summarize the prompt`
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-05 — `E2E-05-chat-translate: analyst → translate hello world`
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-06 — `E2E-06-chat-poem: sr_analyst → quarterly-reports haiku`
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-07 — `E2E-07-chat-explain: manager → bills receivable vs payable`
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-08 — `E2E-08-chat-list: manager → ops manager qualities list`
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-09 — `E2E-09-chat-define: sr_manager → inventory turnover definition`
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-10 — `E2E-10-chat-followup-pretrained-knowledge: director → PO lifecycle`
...
#### [PRODUCT_RUNTIME] E2E-21 — `E2E-21-nxs-sales-list-orders: analyst → last 7 days of sales orders`

- state: `fail`
- reason: NXS dispatch bridge returns null on the connector path (see E2E-23). Scenario: analyst sales-orders 7-day list.
...
#### [PRODUCT_RUNTIME] E2E-22 — `E2E-22-nxs-sales-by-customer: analyst → acme-corp orders last month`

- state: `fail`
- reason: NXS dispatch bridge returns null on the connector path (see E2E-23). Scenario: analyst sales by-customer query.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#11`
- suspectedRootCause: scripts/nexus-main.ts NXS dispatch bridge — finalOutcome=error_dispatch, evidence had no executionResult
...
#### [UNCLASSIFIED] — — `E2E-23-nxs-warehouse-inventory-snapshot: sr_analyst → WIDGET-A qty on hand`

- state: `fail`
- raw error (needs triage):
  ```
  at least one nxs_action with finalOutcome=executed (warehouse connector actually ran): expected 0 to be greater than 0
  ```

#### [PRODUCT_RUNTIME] E2E-24 — `E2E-24-nxs-warehouse-low-stock: sr_analyst → SKUs under 50 qty`

- state: `fail`
- reason: NXS dispatch bridge returns null on the connector path (see E2E-23). Scenario: sr_analyst warehouse low-stock query.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#11`
- suspectedRootCause: scripts/nexus-main.ts NXS dispatch bridge — finalOutcome=error_dispatch, evidence had no executionResult
...
#### [PRODUCT_RUNTIME] E2E-25 — `E2E-25-nxs-sales-top-customers: manager → top 10 by revenue last quarter`

- state: `fail`
- reason: NXS dispatch bridge returns null on the connector path (see E2E-23). Scenario: manager top-customers report.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#11`
- suspectedRootCause: scripts/nexus-main.ts NXS dispatch bridge — finalOutcome=error_dispatch, evidence had no executionResult
...
#### [PRODUCT_RUNTIME] E2E-26 — `E2E-26-nxs-sales-bulk-pull: sr_manager → Q1 export bulk pull`

- state: `fail`
- reason: NXS dispatch bridge returns null on the connector path (see E2E-23). Scenario: sr_manager bulk export — risk-ceiling test.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#10`
- suspectedRootCause: scripts/nexus-main.ts NXS dispatch bridge — finalOutcome=error_dispatch, evidence had no executionResult
...
#### [PRODUCT_RUNTIME] E2E-27 — `E2E-27-nxs-warehouse-reorder-list: director → reorder this week by velocity`

- state: `fail`
- reason: NXS dispatch bridge returns null on the connector path (see E2E-23). Scenario: director reorder by velocity.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#11`
- suspectedRootCause: scripts/nexus-main.ts NXS dispatch bridge — finalOutcome=error_dispatch, evidence had no executionResult
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-28 — `E2E-28-nxs-sales-delete-denied-for-analyst: analyst → delete ORD-12345 → Gate 03 denied`

- state: `fail`
- reason: Gate 03 denial fires BEFORE bridge dispatch, so this test does NOT need the bridge fixed. Body not written.
- blockedBy: `E2E-RBAC-DIFFERENTIALS-CATALOG`
- owner: builder
- lawPins: `HL#5`
- nextRecommendedAction: Post analyst delete run against sales-orders; assert Gate 03 denial with capability_missing BEFORE any connector call.

#### [PRODUCT_RUNTIME] E2E-29 — `E2E-29-nxs-warehouse-update-bulk-allowed-for-manager: manager → mark batch shipped`

- state: `fail`
- reason: NXS dispatch bridge returns null on the connector path (see E2E-23). Scenario: manager bulk update.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#11`
- suspectedRootCause: scripts/nexus-main.ts NXS dispatch bridge — finalOutcome=error_dispatch, evidence had no executionResult
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-31 — `E2E-31-multi-chat-fan-out: sr_analyst → analyst-bot + summary-bot`

- state: `fail`
- reason: Multi-agent no-contract DAG dispatch is unproven via HTTP E2E; needs harness work + sample agents. Scenario: sr_analyst fans out chat to two on-prem chat agents.
- blockedBy: `E2E-MULTI-NO-CONTRACT-CATALOG`
- owner: builder
- lawPins: `HL#8`, `HL#11`
- nextRecommendedAction: Build harness helper that posts a 2-node subTasks DAG with two chat agents, then assert: per-agent mailbox isolation, per-agent NVG event, compile multi-item bundle, no contract template invocation.

#### [UNIMPLEMENTED_TEST_BODY] E2E-32 — `E2E-32-multi-domain-experts: manager → finance-bot + ops-bot`

- state: `fail`
- reason: Multi-agent no-contract DAG dispatch is unproven via HTTP E2E; needs harness work + sample agents. Scenario: manager fan-out to two domain-expert agents.
- blockedBy: `E2E-MULTI-NO-CONTRACT-CATALOG`
- owner: builder
- lawPins: `HL#8`, `HL#11`
- nextRecommendedAction: Build harness helper that posts a 2-node subTasks DAG with two chat agents, then assert: per-agent mailbox isolation, per-agent NVG event, compile multi-item bundle, no contract template invocation.

#### [UNIMPLEMENTED_TEST_BODY] E2E-33 — `E2E-33-multi-language-pair: sr_manager → english-bot + french-bot`

- state: `fail`
- reason: Multi-agent no-contract DAG dispatch is unproven via HTTP E2E; needs harness work + sample agents. Scenario: sr_manager fan-out to two language-pair agents.
- blockedBy: `E2E-MULTI-NO-CONTRACT-CATALOG`
- owner: builder
- lawPins: `HL#8`, `HL#11`
- nextRecommendedAction: Build harness helper that posts a 2-node subTasks DAG with two chat agents, then assert: per-agent mailbox isolation, per-agent NVG event, compile multi-item bundle, no contract template invocation.

#### [UNIMPLEMENTED_TEST_BODY] E2E-34 — `E2E-34-multi-tone: director → formal-bot + casual-bot`

- state: `fail`
- reason: Multi-agent no-contract DAG dispatch is unproven via HTTP E2E; needs harness work + sample agents. Scenario: director fan-out — tone differential.
- blockedBy: `E2E-MULTI-NO-CONTRACT-CATALOG`
- owner: builder
- lawPins: `HL#8`, `HL#11`
- nextRecommendedAction: Build harness helper that posts a 2-node subTasks DAG with two chat agents, then assert: per-agent mailbox isolation, per-agent NVG event, compile multi-item bundle, no contract template invocation.

#### [UNIMPLEMENTED_TEST_BODY] E2E-35 — `E2E-35-multi-judge-format: director → response-bot + judge-bot`

- state: `fail`
- reason: Multi-agent no-contract DAG dispatch is unproven via HTTP E2E; needs harness work + sample agents. Scenario: director response + judge.
- blockedBy: `E2E-MULTI-NO-CONTRACT-CATALOG`
- owner: builder
- lawPins: `HL#8`, `HL#11`
- nextRecommendedAction: Build harness helper that posts a 2-node subTasks DAG with two chat agents, then assert: per-agent mailbox isolation, per-agent NVG event, compile multi-item bundle, no contract template invocation.

#### [UNIMPLEMENTED_TEST_BODY] E2E-36 — `E2E-36-multi-fact-fact-judge: vp → fact1-bot + fact2-bot + judge-bot`

- state: `fail`
- reason: Multi-agent no-contract DAG dispatch is unproven via HTTP E2E; needs harness work + sample agents. Scenario: vp three-agent fact + fact + judge.
- blockedBy: `E2E-MULTI-NO-CONTRACT-CATALOG`
- owner: builder
- lawPins: `HL#8`, `HL#11`
- nextRecommendedAction: Build harness helper that posts a 2-node subTasks DAG with two chat agents, then assert: per-agent mailbox isolation, per-agent NVG event, compile multi-item bundle, no contract template invocation.
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-38 — `E2E-38-multi-translate-pair: sr_manager → en-fr + en-es`

- state: `fail`
- reason: Multi-agent no-contract DAG dispatch is unproven via HTTP E2E; needs harness work + sample agents. Scenario: sr_manager translate pair.
- blockedBy: `E2E-MULTI-NO-CONTRACT-CATALOG`
- owner: builder
- lawPins: `HL#8`, `HL#11`
- nextRecommendedAction: Build harness helper that posts a 2-node subTasks DAG with two chat agents, then assert: per-agent mailbox isolation, per-agent NVG event, compile multi-item bundle, no contract template invocation.

#### [UNIMPLEMENTED_TEST_BODY] E2E-39 — `E2E-39-multi-perspective-shootout: vp → exec/analyst/intern perspectives`

- state: `fail`
- reason: Multi-agent no-contract DAG dispatch is unproven via HTTP E2E; needs harness work + sample agents. Scenario: vp three-agent perspective shootout.
- blockedBy: `E2E-MULTI-NO-CONTRACT-CATALOG`
- owner: builder
- lawPins: `HL#8`, `HL#11`
- nextRecommendedAction: Build harness helper that posts a 2-node subTasks DAG with two chat agents, then assert: per-agent mailbox isolation, per-agent NVG event, compile multi-item bundle, no contract template invocation.

#### [UNIMPLEMENTED_TEST_BODY] E2E-40 — `E2E-40-multi-no-contract-bundle-shape: executive → bundle FinalResponseArtifact carries both items`

- state: `fail`
- reason: Multi-agent no-contract DAG dispatch is unproven via HTTP E2E; needs harness work + sample agents. Scenario: executive bundle-shape assertion — two mailbox items → one multi-item artifact.
- blockedBy: `E2E-MULTI-NO-CONTRACT-CATALOG`
- owner: builder
- lawPins: `HL#11`, `F4.12`
- nextRecommendedAction: Build harness helper that posts a 2-node subTasks DAG with two chat agents, then assert: per-agent mailbox isolation, per-agent NVG event, compile multi-item bundle, no contract template invocation.
...
#### [PRODUCT_RUNTIME] E2E-71 — `E2E-71-batch-50-orders: analyst → 50 sales orders → one-paragraph summary`

- state: `fail`
- reason: Batch 50 sales-order rows cannot run until the NXS dispatch bridge returns a real executionResult (see E2E-23). nxs_action emits finalOutcome=error_dispatch, then bridge returns null.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#11`
- suspectedRootCause: scripts/nexus-main.ts NXS dispatch bridge — Gates 01-07 run but executionResult is not propagated back; bridge sees null and emits nxs_dispatch_bridge_returned_null.
- nextRecommendedAction: Fix the bridge first. Once E2E-23 passes, build batch-pull harness helper, then write each scenario body.

#### [PRODUCT_RUNTIME] E2E-72 — `E2E-72-batch-100-invoices: manager → 100 invoices → top 5 anomalies`

- state: `fail`
- reason: Batch 100 invoices cannot run until the NXS dispatch bridge returns a real executionResult (see E2E-23). nxs_action emits finalOutcome=error_dispatch, then bridge returns null.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#11`
- suspectedRootCause: scripts/nexus-main.ts NXS dispatch bridge — Gates 01-07 run but executionResult is not propagated back; bridge sees null and emits nxs_dispatch_bridge_returned_null.
...
#### [PRODUCT_RUNTIME] E2E-73 — `E2E-73-batch-warehouse-receipts: sr_analyst → 30 days receipts → shortage forecast`

- state: `fail`
- reason: Batch 30 days warehouse receipts cannot run until the NXS dispatch bridge returns a real executionResult (see E2E-23). nxs_action emits finalOutcome=error_dispatch, then bridge returns null.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#11`
- suspectedRootCause: scripts/nexus-main.ts NXS dispatch bridge — Gates 01-07 run but executionResult is not propagated back; bridge sees null and emits nxs_dispatch_bridge_returned_null.
...
#### [PRODUCT_RUNTIME] E2E-74 — `E2E-74-batch-customer-tickets: sr_manager → 200 tickets → category breakdown`

- state: `fail`
- reason: Batch 200 customer tickets cannot run until the NXS dispatch bridge returns a real executionResult (see E2E-23). nxs_action emits finalOutcome=error_dispatch, then bridge returns null.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#11`
- suspectedRootCause: scripts/nexus-main.ts NXS dispatch bridge — Gates 01-07 run but executionResult is not propagated back; bridge sees null and emits nxs_dispatch_bridge_returned_null.
...
#### [PRODUCT_RUNTIME] E2E-75 — `E2E-75-batch-merged-files: director → sales(50)+warehouse(50) → merge → summary`

- state: `fail`
- reason: Batch sales+warehouse merge cannot run until the NXS dispatch bridge returns a real executionResult (see E2E-23). nxs_action emits finalOutcome=error_dispatch, then bridge returns null.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#8`, `HL#11`
- suspectedRootCause: scripts/nexus-main.ts NXS dispatch bridge — Gates 01-07 run but executionResult is not propagated back; bridge sees null and emits nxs_dispatch_bridge_returned_null.
...
#### [PRODUCT_RUNTIME] E2E-77 — `E2E-77-batch-classified: director → 50 confidential + 50 internal → NVG case-split`

- state: `fail`
- reason: Batch mixed-classification batch cannot run until the NXS dispatch bridge returns a real executionResult (see E2E-23). nxs_action emits finalOutcome=error_dispatch, then bridge returns null.
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
...
#### [PRODUCT_RUNTIME] E2E-81 — `E2E-81-quarterly-coverage: sr_manager → can we cover Q2 orders?`

- state: `fail`
- reason: Multi-source merge sales orders + warehouse inventory requires two successful NXS pulls; bridge currently returns null (see E2E-23).
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#8`, `HL#11`
- nextRecommendedAction: Fix bridge. Then build merge harness helper: post a DAG with two NXS pull nodes feeding a compile node; assert per-actor mailbox isolation and merge correctness.

#### [PRODUCT_RUNTIME] E2E-82 — `E2E-82-stockout-risk: manager → SKUs at risk this month`

- state: `fail`
- reason: Multi-source merge sales velocity + warehouse stock requires two successful NXS pulls; bridge currently returns null (see E2E-23).
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#8`, `HL#11`
- nextRecommendedAction: Fix bridge. Then build merge harness helper: post a DAG with two NXS pull nodes feeding a compile node; assert per-actor mailbox isolation and merge correctness.

#### [PRODUCT_RUNTIME] E2E-83 — `E2E-83-reorder-recommendations: sr_manager → reorder plan`

- state: `fail`
- reason: Multi-source merge sales + warehouse + reorder thresholds requires two successful NXS pulls; bridge currently returns null (see E2E-23).
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#8`, `HL#11`
- nextRecommendedAction: Fix bridge. Then build merge harness helper: post a DAG with two NXS pull nodes feeding a compile node; assert per-actor mailbox isolation and merge correctness.

#### [PRODUCT_RUNTIME] E2E-84 — `E2E-84-customer-shipping-perf: manager → on-time shipping report`

- state: `fail`
- reason: Multi-source merge sales orders + warehouse shipments requires two successful NXS pulls; bridge currently returns null (see E2E-23).
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#8`, `HL#11`
- nextRecommendedAction: Fix bridge. Then build merge harness helper: post a DAG with two NXS pull nodes feeding a compile node; assert per-actor mailbox isolation and merge correctness.

#### [PRODUCT_RUNTIME] E2E-85 — `E2E-85-sku-margin-analysis: director → margin-by-SKU`

- state: `fail`
- reason: Multi-source merge sales prices + warehouse cost requires two successful NXS pulls; bridge currently returns null (see E2E-23).
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#8`, `HL#11`
- nextRecommendedAction: Fix bridge. Then build merge harness helper: post a DAG with two NXS pull nodes feeding a compile node; assert per-actor mailbox isolation and merge correctness.
...
#### [PRODUCT_RUNTIME] E2E-87 — `E2E-87-anomaly-detect: sr_manager → refunds vs returns anomalies`

- state: `fail`
- reason: Multi-source merge refund returns anomaly join requires two successful NXS pulls; bridge currently returns null (see E2E-23).
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#8`, `HL#11`
- nextRecommendedAction: Fix bridge. Then build merge harness helper: post a DAG with two NXS pull nodes feeding a compile node; assert per-actor mailbox isolation and merge correctness.

#### [PRODUCT_RUNTIME] E2E-88 — `E2E-88-supply-chain-snapshot: director → board snapshot`

- state: `fail`
- reason: Multi-source merge cross-system board snapshot requires two successful NXS pulls; bridge currently returns null (see E2E-23).
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#8`, `HL#11`
- nextRecommendedAction: Fix bridge. Then build merge harness helper: post a DAG with two NXS pull nodes feeding a compile node; assert per-actor mailbox isolation and merge correctness.

#### [PRODUCT_RUNTIME] E2E-89 — `E2E-89-margin-by-region: vp → margin per region`

- state: `fail`
- reason: Multi-source merge sales + region tagging + cost requires two successful NXS pulls; bridge currently returns null (see E2E-23).
- blockedBy: `NXS-DISPATCH-BRIDGE-RETURNS-NULL`
- owner: arch
- lawPins: `HL#5`, `HL#8`, `HL#11`
- nextRecommendedAction: Fix bridge. Then build merge harness helper: post a DAG with two NXS pull nodes feeding a compile node; assert per-actor mailbox isolation and merge correctness.
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-111 — `E2E-111-hl1-auth-first: no auth → 401 before any gate`

- state: `fail`
- reason: HL#1 auth-first. Body not written. Testable now — POST /workspace/runs with no JWT, assert 401, assert NO ledger event fires.
- blockedBy: `E2E-HARD-LAW-CATALOG`
- owner: builder
- lawPins: `HL#1`
- nextRecommendedAction: fetch POST /workspace/runs with no Authorization header → expect 401, then assert no run was opened in the run ledger.
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-113 — `E2E-113-hl5-nxs-only-action-auth: agent NXS-bypass via side-channel → blocked`

- state: `fail`
- reason: HL#5 — NXS is the sole action authority. Body should attempt to call a connector directly (no run) and assert refusal at boundary.
- blockedBy: `E2E-HARD-LAW-CATALOG`
- owner: builder
- lawPins: `HL#5`

#### [UNIMPLEMENTED_TEST_BODY] E2E-114 — `E2E-114-hl6-nvg-sole-llm-auth: agent direct LLM call → blocked`

- state: `fail`
- reason: HL#6 — NVG is the sole LLM authority. Body should attempt to call a model adapter directly and assert refusal.
- blockedBy: `E2E-HARD-LAW-CATALOG`
- owner: builder
- lawPins: `HL#6`

#### [UNIMPLEMENTED_TEST_BODY] E2E-115 — `E2E-115-hl7-llm-no-tool-descriptors: nvg_outbound payload has empty toolDescriptors`

- state: `fail`
- reason: HL#7 — model never sees tool descriptors. Body should run any LLM-calling test and assert nvg_outbound event has toolDescriptors: [].
- blockedBy: `E2E-HARD-LAW-CATALOG`
- owner: builder
- lawPins: `HL#7`
- nextRecommendedAction: Reuse E2E-01 chat flow, read the ledger nvg_outbound event, assert toolDescriptors is an empty array (planner-drift gate).

#### [UNIMPLEMENTED_TEST_BODY] E2E-116 — `E2E-116-hl8-mailbox-only-data-hub: cross-actor mailbox read denied`

- state: `fail`
- reason: HL#8 — mailbox is the only data hub. Body should attempt cross-actor mailbox read and assert refusal.
- blockedBy: `E2E-HARD-LAW-CATALOG`
- owner: builder
- lawPins: `HL#8`
- nextRecommendedAction: Open run A as actor X, attempt to read run A mailbox via actor Y JWT, assert 403/empty.

#### [UNIMPLEMENTED_TEST_BODY] E2E-117 — `E2E-117-hl11-compile-passthrough: single agent + no contract = verbatim forward`

- state: `fail`
- reason: HL#11 — compile pass-through. Body should run a single-agent no-contract chat, then assert the FinalResponseArtifact body equals the agent mailbox item verbatim (digest match).
- blockedBy: `E2E-HARD-LAW-CATALOG`
- owner: builder
- lawPins: `HL#11`
- nextRecommendedAction: Extend E2E-01 — after run closes, read mailbox item digest + final_response artifact digest; assert equal.
...
#### [UNIMPLEMENTED_TEST_BODY] E2E-119 — `E2E-119-hl15-symmetric-intersection: real per-dimension differential`

- state: `fail`
- reason: HL#15 — effective scope is intersection(user, agent, delegation). Body needs to register a narrow agent + delegate broader than the agent, then assert the agent ceiling wins.
- blockedBy: `E2E-HARD-LAW-CATALOG`
- owner: builder
- lawPins: `HL#15`
- nextRecommendedAction: Register agent with allowedSystems=[sales]; delegate with allowedSystems=[sales,warehouse]; run against warehouse; assert Gate 03 denial.

#### [UNIMPLEMENTED_TEST_BODY] E2E-120 — `E2E-120-hl16-agent-touches-only-mailboxes: agent → NXS/NVG/ledger direct call blocked`

- state: `fail`
- reason: HL#16 — agent boundary. Body should prove there is no HTTP path from an agent to NXS/NVG/ledger directly, only through the orch + mailbox.
- blockedBy: `E2E-HARD-LAW-CATALOG`
- owner: builder
- lawPins: `HL#16`
- nextRecommendedAction: Enumerate registered agent endpoints; assert none expose a route that bypasses the orch + mailbox pipeline.
